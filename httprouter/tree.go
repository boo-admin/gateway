...
// this is modified file at https://github.com/gin-gonic/gin/blob/master/tree.go
package httprouter
...
func (n *node) insertChild(path string, fullPath string, handle HandleFunc) {
	for {
	...
	for {
		...
		if len(path) > len(prefix) {
			if path[:len(prefix)] == prefix {
				...
				case param:
					...
					if value.handle = n.handle; value.handle != nil {
					...
					if len(n.children) == 1 {
						...
						n = n.children[0]
						value.tsr = (n.path == "/" && n.handle != nil) || (n.path == "" && n.indices == "/")
					}
					return value
		...
		if path == prefix {
			...
			for i, c := range []byte(n.indices) {
				if c == '/' {
					n = n.children[i]
					value.tsr = (len(n.path) == 1 && n.handle != nil) ||
